...
angular.module('classDigServices', [])
    ...
    .factory('activeUser',
        ['$resource',
            'appSettings',
            '$http',
            function ($resource, appSettings, $http, $rootScope) {
                var usersList = [];
                    ...
                    setUsersList: function (id) {
                        $http({
                            ...
                            .then(function (response) {
                                    var listUsers = response.data.follows;
                                    for(var i=0; i<listUsers.length; i++){
                                        usersList.push(listUsers[i]);
                                    }
    ...
    .factory('Users', function ($http, appSettings, $log, $rootScope) {
        ...
        Users.prototype.nextPage = function (digClass, selectedRole, array) {
            ...
            this.request = $http.get(url).success(function (data) {
                ...
                that.busy = false;
                if (array) {
                    $rootScope.$broadcast('u-can-render')
                }
                $('#user-loader').hide();
            ...
            if (_.isEmpty(obj.pickedClass)) {
                $http.get(appSettings.link + 'class/' + $routeParams.classId)
                    .success(function (response) {
                        ...
                        if (obj.pickedClass.status !== 2) {
                            obj.pickedClass.classInArchived = false;
                        } else {
                            obj.pickedClass.classInArchived = true;
                        }
                        cb(obj.pickedClass);
                    })
                    .error(function (data) {
